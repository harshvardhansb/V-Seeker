                    ...
                    if (myActiveApplicationCount < 10) {
                      Application.countDocuments({
                        ...
                        if (acceptedJobs === 0) {
                          ...
                          res.status(400).json({
                            message:
                              "You already have an accepted . Hence you cannot apply.",
                          });
                        }
                      });
                    } else {
          ...
          Job.findOne({
            ...
            Application.countDocuments({
              ...
              if (activeApplicationCount < job.maxPositions) {
                ...
                res.status(400).json({
                  message: "All positions for this opportunity are already filled",
                });
              }
            });
          });
    ...
    Application.aggregate([
      ...
      .then((applications) => {
        if (applications.length === 0) {
          res.status(404).json({
            message: "No volunteering applicants found",
          });
          return;
        }
        res.json(applications);
          ...
          Application.countDocuments({
            ...
            .then((acceptedApplicant) => {
              if (acceptedApplicant > 0) {
                ...
                res.status(400).json({
                  message:
                    "You haven't worked for this organisation. Hence you cannot give a rating.",
                });
              }
            })
            .catch((err) => {
                ...
                .then((result) => {
                  ...
                  Job.findOneAndUpdate(
                    ...
                    .then((foundJob) => {
                      if (foundJob === null) {
                        res.status(400).json({
                          message: "Error while updating average rating",
                        });
                        return;
                      }
                      res.json({
